...
async function loadSettings(){const local=await chrome.storage.local.get([...KEYS,'apiKey']);if(!local.openRouterApiKey&&typeof local.apiKey==='string'&&local.apiKey.trim())local.openRouterApiKey=local.apiKey.trim();await chrome.storage.local.remove(['apiKey','syncKey','provider','allowRemoteOcr','plan']);await chrome.storage.sync.remove(['apiKey','syncKey']);return validateSettings(local);}
