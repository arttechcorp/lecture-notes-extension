...
const DEFAULTS={serviceUrl:'',appSessionToken:'',summaryModel:'google/gemini-2.5-flash-lite',remoteSummaryConsent:false,ocrEnabled:true,ocrEngine:'tesseract',whisperEnabled:false,whisperModel:'tiny',whisperLang:'auto',outputFormat:'summary',theme:'system',consentAccepted:false};
...
function validateSettings(input={}){
  ...
  if(!['tiny','base'].includes(out.whisperModel))out.whisperModel='tiny';out.whisperLang=({korean:'ko',english:'en'})[out.whisperLang]||out.whisperLang;if(!['auto','ko','en'].includes(out.whisperLang))out.whisperLang='auto';out.ocrEngine='tesseract';if(!['summary','custom'].includes(out.outputFormat))out.outputFormat='summary';if(!['system','light','dark'].includes(out.theme))out.theme='system';
