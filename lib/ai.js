...
async function callRemote(provider, model, apiKey, body, retries = 2) {
  ...
  if (activeKey.startsWith("sk-or-")) {
    ...
    if (!activeModel || activeModel.includes("claude-3-5")) activeModel = "anthropic/claude-sonnet-5";
  } else if (activeKey.startsWith("sk-ant-")) {
    activeProvider = "anthropic";
  } else if (activeKey.startsWith("AIzaSy")) {
    activeProvider = "gemini";
  } else if (activeKey.startsWith("apikey_")) {
    throw new Error(
