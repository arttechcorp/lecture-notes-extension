...
class AudioCapturer {
  ...
  pushAudio({ pcm, t, rms, rate }) {
    ...
    this._log(
      `청크 ${audio.length}샘플, RMS ${rms.toFixed(5)}` +
        (rate && rate !== 1 ? ` (${rate}배속 → 원속도로 복원)` : '')
    );
    this.worker.postMessage({ type: 'TRANSCRIBE', audio, id: t, language: this.language });
  }
