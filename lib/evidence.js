...
(() => {
  ...
  class EvidenceStore {
    ...
    add({ source, time = 0, t0 = time, t1 = t0, text, confidence = null, bbox = null, epoch = 0, slideId = null }) {
    ...
    restore(items) {
      ...
      for (const e of items) {
        ...
        if(!validBox||!Number.isInteger(e.epoch??0)||(e.epoch??0)<0||(e.confidence!=null&&(!Number.isFinite(e.confidence)||e.confidence<0||e.confidence>1)))throw new Error("보관 근거의 좌표 또는 메타데이터가 잘못되었습니다.");
        ids.add(e.id);
