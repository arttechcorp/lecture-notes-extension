...
(() => {
  ...
  const LISTS = ["keyConclusions", "concepts", "claims", "definitions", "relationships", "examples", "corrections", "openQuestions"];
  ...
  function validateSummary(value, entries, { requireCoverage = true, maxItems = 100, maxSections = 80, maxQuestions = 30 } = {}) {
    ...
    out.evidenceIds = citations(value.evidenceIds, ids, "전체 요약");
    if (requireCoverage && [...ids].some(id => !out.evidenceIds.includes(id))) throw new Error("요약에서 처리되지 않은 근거가 있습니다.");
  ...
  async function generate(entries, { sessionId, signal, onProgress = () => {}, settings = {}, service = globalThis.ServiceClient, cache = new Map(), attempt = 0 } = {}) {
    ...
    if (!openRouterApiKey && (!settings.serviceUrl || !token)) return recognitionResult(prepared);
    ...
    const call = async (chunk, stage) => {
      ...
      const response = await service.summary({ apiKey: openRouterApiKey, baseUrl: settings.serviceUrl, token, model, evidence: chunk, requestId: await requestId(sessionId, stage, { model, chunk, attempt }), stage, signal, timeoutMs: 120000 });
      aborted(signal);
    ...
    try {
      ...
      final = { ...final, status: "complete", coverage: { total: chunks.length, completed: chunks.length, failed: 0, synthesis: true }, evidenceRefs: refs, preprocessing: prep, usage: { ...usage } };
      onProgress("전체 학습 노트 합성 완료"); return final;
    } catch (error) {
      error.partial = { ...partialResult(completed, chunks), evidenceRefs: refs, preprocessing: prep, usage: { ...usage } };
      throw error;
    }
  }
  const api = { chunkEvidence, preprocessEvidence, validateSummary, generate, recognitionResult, requestId, remapSummary };
