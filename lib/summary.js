...
(() => {
  ...
  async function generate(entries, { sessionId, signal, onProgress = () => {}, settings = {}, service = globalThis.ServiceClient, cache = new Map(), attempt = 0, gaps = [] } = {}) {
    ...
    try {
      ...
      final = { ...final, notice: [notice, droppedNotice(droppedItems), uncitedNotice(uncitedAll)].filter(Boolean).join("\n"), dropped: droppedItems, uncited: uncitedAll, status: "complete", coverage: { total: chunks.length, completed: chunks.length, failed: 0, synthesis: true }, evidenceRefs: refs, preprocessing: prep, usage: { ...usage } };
      onProgress("전체 학습 노트 합성 완료"); return final;
    } catch (error) {
      error.partial = { ...partialResult(completed, chunks), notice: [notice, droppedNotice(droppedItems), uncitedNotice(uncitedAll)].filter(Boolean).join("\n"), dropped: droppedItems, uncited: uncitedAll, evidenceRefs: refs, preprocessing: prep, usage: { ...usage } };
      throw error;
    }
  }
