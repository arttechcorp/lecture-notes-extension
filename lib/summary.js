...
(() => {
  ...
  function item(value, ids, label) {
    ...
    return evidenceIds && { content: value.content, importance: value.importance, evidenceIds };
  }
  ...
  async function generate(entries, { sessionId, signal, onProgress = () => {}, settings = {}, service = globalThis.ServiceClient, cache = new Map(), attempt = 0, gaps = [] } = {}) {
    ...
    try {
      ...
      final = { ...final, notice: [notice, droppedNotice(droppedItems)].filter(Boolean).join("\n"), dropped: droppedItems, status: "complete", coverage: { total: chunks.length, completed: chunks.length, failed: 0, synthesis: true }, evidenceRefs: refs, preprocessing: prep, usage: { ...usage } };
      onProgress("전체 학습 노트 합성 완료"); return final;
    } catch (error) {
      error.partial = { ...partialResult(completed, chunks), notice: [notice, droppedNotice(droppedItems)].filter(Boolean).join("\n"), dropped: droppedItems, evidenceRefs: refs, preprocessing: prep, usage: { ...usage } };
      throw error;
    }
  }
  const api = { gapRanges, gapNotice, droppedNotice, chunkEvidence, preprocessEvidence, validateSummary, generate, recognitionResult, requestId, remapSummary, LISTS };
