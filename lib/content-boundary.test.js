...
test("content metadata preserves a clipped video position for the session guard",()=>{
  ...
  const context=vm.createContext({innerWidth:1280,innerHeight:720,setInterval,clearInterval,setTimeout,clearTimeout,addEventListener:()=>{},document:{querySelectorAll:()=>[video],createElement:()=>canvas},chrome:{runtime:{id:"extension-id",onMessage:{addListener:fn=>{listener=fn;}},sendMessage:async()=>({ok:true})}}});context.window=context;
