(() => {
  ...
  async function request(o,route,method="GET",body){
    ...
    try{
      ...
      if(!response.ok){const messages={unauthorized:"서비스 인증이 만료됐습니다. 설정에서 다시 연결하세요.",quota_exceeded:"이번 달 요약 한도에 도달했습니다.",request_already_reserved_or_processed:"이미 처리했거나 비용이 예약된 요청입니다. 중복 요청은 실행하지 않았습니다.",provider_failed_or_invalid_output:"요약 제공자가 결과를 완료하지 못했습니다. 현재까지의 노트는 유지됩니다.",feature_not_in_account_plan:"현재 요금제에서 지원하지 않는 기능입니다. 설정에서 고화질 화면 인식을 끄세요."};throw new Error(messages[data.error]||"서비스 요청을 완료하지 못했습니다 ("+response.status+").");}
      return data;
