...
(() => {
  ...
  const schema = { type: "object", additionalProperties: false, required: ["title", "keyConclusions", "concepts", "corrections", "openQuestions", "sections", "formulas", "visuals", "reviewQuestions", "evidenceIds"], properties: {
  ...
  const BASE = [
    ...
    "Build a visual whenever the evidence carries comparable numbers, parameters, steps or a structure: type \"table\" and type \"chart\" put a GitHub-flavoured Markdown table of the values in data; type \"relationship\" puts a ```mermaid fenced graph in data. Use only values present in the evidence.",
    ...
    "Every item carries only supplied evidence IDs, and the top-level evidenceIds must cover every input ID.",
  ...
  const MODELS = {
    "google/gemini-2.5-flash-lite": { tags: ["google-vertex"], reasoning: { enabled: false }, maxTokens: 8192 },
    "google/gemini-3.8-flash": { tags: ["google-vertex/global"], reasoning: { effort: "low" }, maxTokens: 16384 },
    "google/gemini-2.5-pro": { tags: ["google-vertex/global"], reasoning: {}, maxTokens: 16384 },
    "anthropic/claude-haiku-4.5": { tags: ["amazon-bedrock/global"], reasoning: { enabled: false }, maxTokens: 8192 },
    "anthropic/claude-sonnet-4.6": { tags: ["amazon-bedrock/global"], reasoning: { enabled: false }, maxTokens: 8192 },
  };
  ...
  async function summary(options) {
    ...
    const data = await request({ ...options, route: URL, method: "POST", body: {
      ...
      messages: [{ role: "system", content: systemFor(options.stage) }, { role: "user", content: JSON.stringify({ stage: options.stage || "chunk", evidence: options.evidence, ...(options.gaps?.length ? { gaps: options.gaps } : {}) }) }],
  ...
  const api = { summary, check, schema, systemFor, reasoningFor, maxTokensFor, MODELS };
