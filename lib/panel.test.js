...
// 4. "요약만 보내기"에서 로컬 요약기가 없으면 원격으로 넘어가지 않아야 한다.
//    이 가드가 사라지면 프라이버시 설정이 조용히 약해진다 — 눈에 안 보이는 고장이다.
assert.ok(
  /scope === "summary" && !localReady/.test(js) && /condenseLocal|condense\(/.test(js),
  "generateNotes 에 '요약만 보내기' 가드나 로컬 축약 단계가 없다"
);

const guardAt = js.indexOf('scope === "summary" && !localReady');
const remoteAt = js.indexOf("await notesRemote(");
assert.ok(guardAt > 0 && remoteAt > guardAt, "가드가 원격 호출보다 뒤에 있다 — 순서가 뒤집혔다");
