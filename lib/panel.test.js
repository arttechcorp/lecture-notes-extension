...
assert.match(html, /id="notionModal"/, "notionModal 다이얼로그가 sidepanel.html 에 없다");
assert.match(html, /id="notionModalClose"/, "notionModalClose 버튼이 sidepanel.html 에 없다");
assert.match(html, /allow-modals/, "renderFrame sandbox 속성에 allow-modals 가 없다");
assert.match(html, /복사 완료! 이제 노션에 복사하셔서 사용하시면 됩니다!/, "노션 복사 안내 문구가 sidepanel.html 에 없다");
...
assert.match(js, /\bnotionModal:\s*\$\("notionModal"\)/, "els.notionModal 매핑이 없다");
assert.match(js, /\bnotionModalClose:\s*\$\("notionModalClose"\)/, "els.notionModalClose 매핑이 없다");
assert.match(js, /복사 완료! 이제 노션에 복사하셔서 사용하시면 됩니다!/, "노션 복사 안내 문구가 sidepanel.js 에 없다");

// 6. 실시간 오디오 버퍼 수집 안내 및 노트 생성 로딩 오버레이 확인
assert.match(html, /id="noteLoadingOverlay"/, "noteLoadingOverlay 가 sidepanel.html 에 없다");
assert.match(html, /id="noteLoadingTitle"/, "noteLoadingTitle 이 sidepanel.html 에 없다");
assert.match(html, /id="overlayPlanLink"/, "overlayPlanLink 가 sidepanel.html 에 없다");
assert.match(html, /노트 필기중\.\.\.\./, "노트 필기중.... 문구가 sidepanel.html 에 없다");
assert.match(html, /필기를 더 빠르게 하고 싶으신가요\?/, "필기를 더 빠르게 하고 싶으신가요? 문구가 sidepanel.html 에 없다");
assert.match(js, /\bnoteLoadingOverlay:\s*\$\("noteLoadingOverlay"\)/, "els.noteLoadingOverlay 매핑이 없다");
assert.match(js, /\bnoteLoadingTitle:\s*\$\("noteLoadingTitle"\)/, "els.noteLoadingTitle 매핑이 없다");
assert.match(js, /\boverlayPlanLink:\s*\$\("overlayPlanLink"\)/, "els.overlayPlanLink 매핑이 없다");
assert.match(js, /오디오 버퍼 수집중\.\.\./, "오디오 버퍼 수집중... 문구가 sidepanel.js 에 없다");
assert.match(js, /머릿속을 정리중이에요/, "머릿속을 정리중이에요 문구가 sidepanel.js 에 없다");

// 7. 온디바이스 Nano 프롬프트 최적화 및 렌더링 fallback 확인
assert.match(js, /시각화\/다이어그램 금지/, "Nano용 Mermaid 금지 지침이 sidepanel.js 에 없다");
assert.match(js, /이미지 금지/, "Nano용 이미지 금지 지침이 sidepanel.js 에 없다");
const sandboxHtml = fs.readFileSync(path.join(__dirname, "..", "sandbox.html"), "utf8");
assert.match(sandboxHtml, /broken-image-fallback/, "깨진 이미지 fallback 처리가 sandbox.html 에 없다");
assert.match(sandboxHtml, /mermaid-fallback-box/, "Mermaid 렌더링 실패 fallback 처리가 sandbox.html 에 없다");
