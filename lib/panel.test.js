...
const note={title:'강의',evidenceRefs:[ref('a',10),ref('b',20),ref('c',30),ref('d',40)],
  ...
  concepts:[cite('A개념','important',['b']),cite('B개념','reference',['d']),cite('C개념','critical',['b'])],
...
// critical 항목은 형광펜(==…==)으로 칠하고, 그 아래 중요도는 칠하지 않는다.
assert.match(noteText(note),/- ==C개념==/,'critical 항목에 형광펜이 사라졌다');
assert.doesNotMatch(noteText(note),/==A개념==|==B개념==/,'critical 이 아닌 항목까지 칠하고 있다');
// 핵심 결론은 거의 전부 critical 이라 칠하면 섹션이 통째로 칠해진다 — 제목이 이미 그 역할을 한다.
assert.doesNotMatch(noteText(note),/==결론==/,'핵심 결론에 형광펜이 다시 붙었다');
// 핵심 결론은 인용(>) 콜아웃 상자로 나간다 — 바탕색은 product-panel.css 의 blockquote 규칙이 칠한다.
// detail 이 있으면 결론 문장 아래 문단으로 붙는다.
const boxed=noteText({...note,keyConclusions:[{...cite('결론','critical',['c']),detail:'자세한 설명'}]});
assert.ok(boxed.includes(['> **결론**','> ','> 자세한 설명'].join(String.fromCharCode(10))),'핵심 결론 콜아웃에 자세한 설명이 붙지 않았다');
assert.ok(noteText(note).includes('> **결론**'),'detail 없는 핵심 결론도 콜아웃으로 나가야 한다');
// 빈 줄이 든 내용은 문단이 갈려 ==가 짝을 잃고 원문에 그대로 남는다.
assert.doesNotMatch(noteText({...note,concepts:[cite('앞\n\n뒤','critical',['b'])]}),/==/,'문단이 갈리는 내용까지 칠하고 있다');
