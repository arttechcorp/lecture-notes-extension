...
// 5. 출력 형태가 요약을 통째로 건너뛰게 해서는 안 된다.
//    "원문 타임라인"이 select 의 첫 항목이면서 generateNotes 맨 앞에서 조기 반환을
//    걸고 있었다. 그래서 Premium/Sonnet 을 골라도 요약이 한 번도 실행되지 않았다.
//    원문 보기는 결과 화면의 타임라인 버튼이 이미 담당한다.
assert.ok(
  !/value="timeline"/.test(html),
  "출력 형태에 timeline 선택지가 있다 — 요약을 건너뛰는 함정이 돌아왔다"
);
const gen = js.slice(js.indexOf("async function generateNotes"));
assert.ok(
  !/outputFormat\.value === "timeline"/.test(gen),
  "generateNotes 가 출력 형태만 보고 조기 반환한다 — 요약이 실행되지 않는다"
);
