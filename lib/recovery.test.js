...
const local={apiKey:'sk-or-v1-'+'a'.repeat(32),provider:'gemini',serviceUrl:'https://service.example',appSessionToken:'token'};
...
(async()=>{const s=await vm.runInContext('loadSettings()',scope);assert.equal(s.serviceUrl,'https://service.example');assert.equal(s.openRouterApiKey,'sk-or-v1-'+'a'.repeat(32));assert.equal(local.apiKey,undefined);assert.equal(await vm.runInContext("saveSettings({transcript:'bad'})",scope).catch(()=> 'rejected'),'rejected');console.log('recovery: legacy credential migration passed');})().catch(e=>{console.error(e);process.exitCode=1;});
