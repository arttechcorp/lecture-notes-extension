...
test('reasoning and token budget follow the model, not one global default',async()=>{
  ...
  assert.deepEqual(sent['google/gemini-3.8-flash'],[{effort:'low'},32768]);
  assert.deepEqual(sent['google/gemini-2.5-pro'],[{},32768]);
  assert.deepEqual(sent['anthropic/claude-sonnet-4.6'],[{enabled:false},32768]);
  assert.deepEqual(sent['google/gemini-2.5-flash-lite'],[{enabled:false},32768]);
});
...
// JS 소스의 '\\times' (백슬래시 두 개)는 JSON 텍스트에서 '\times' (백슬래시 한 개)가 되어,
// 모델이 더블 이스케이프를 빼먹었을 때 실제로 API가 보내는 손상과 정확히 같은 바이트열이 된다.
test("응답 쪽 백슬래시 손실이 복구된다(\\t \\f)",async()=>{
  const content=JSON.stringify(summary).replace('"요약"','"1.05mA \\times 2k \\frac{1}{R}"');
  const result=await OpenRouter.summary({apiKey:key,model:'google/gemini-2.5-flash-lite',stage:'chunk',evidence,fetcher:async()=>response({choices:[{finish_reason:'stop',message:{content}}],usage:{}})});
  assert.match(result.summary.title,/\\times/);
  assert.match(result.summary.title,/\\frac\{1\}\{R\}/);
  assert.ok(!result.summary.title.includes('\t'),'TAB이 남아있으면 복구 실패');
  assert.ok(!result.summary.title.includes('\f'),'FORMFEED이 남아있으면 복구 실패');
});
test("이미 이스케이프된 LaTeX은 그대로 유지된다(이중 복구 금지)",async()=>{
  const content=JSON.stringify(summary).replace('"요약"','"\\\\times"');
  const result=await OpenRouter.summary({apiKey:key,model:'google/gemini-2.5-flash-lite',stage:'chunk',evidence,fetcher:async()=>response({choices:[{finish_reason:'stop',message:{content}}],usage:{}})});
  assert.equal(result.summary.title,'\\times');
});
test("줄바꿈 뒤 eq는 건드리지 않는다(\\n은 복구 대상이 아니다)",async()=>{
  const content=JSON.stringify(summary).replace('"요약"','"a\\neq b"');
  const result=await OpenRouter.summary({apiKey:key,model:'google/gemini-2.5-flash-lite',stage:'chunk',evidence,fetcher:async()=>response({choices:[{finish_reason:'stop',message:{content}}],usage:{}})});
  assert.equal(result.summary.title,'a\neq b');
});
...
test('캐시 중단점은 Anthropic 엔드포인트에만 찍고 system 본문은 같다',async()=>{
  const gemini=OpenRouter.systemMessage('google/gemini-2.5-flash-lite','chunk');
  const claude=OpenRouter.systemMessage('anthropic/claude-haiku-4.5','synthesis');
  assert.equal(gemini.content,OpenRouter.systemFor('chunk'),'암묵 캐시 모델은 예전처럼 문자열을 받는다');
  assert.deepEqual(claude.content,[{type:'text',text:OpenRouter.systemFor('synthesis'),cache_control:{type:'ephemeral'}}]);
  let body;
  await OpenRouter.summary({apiKey:key,model:'anthropic/claude-haiku-4.5',stage:'chunk',evidence,fetcher:async(url,options)=>{body=JSON.parse(options.body);return response({choices:[{finish_reason:'stop',message:{content:JSON.stringify(summary)}}],usage:{}});}});
  assert.equal(body.messages[0].content[0].cache_control.type,'ephemeral','실제 요청에 중단점이 실리지 않는다');
  assert.equal(body.messages[0].content[0].text,OpenRouter.systemFor('chunk'));
});
