...
(async () => {
  ...
  const { summarizeLocal, LOCAL_SUMMARY_TIMEOUT_MS } = require("./ai.js");
  ...
  // promptStreaming 지원 세션: 스트리밍 청크 누적 및 진행률 콜백 검증
  ...
  const resStream = await summarizeLocal(
    mockStreamingSession,
    "짧은 강의",
    (t) => t,
    (msg) => progressUpdates.push(msg)
  );
  assert.strictEqual(resStream, "핵심 개념 정리");
  assert.ok(progressUpdates.some((msg) => msg.includes("자")));
