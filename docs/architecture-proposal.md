...
작성일: 2026-09-10. 최근 수정: 2026-09-13. 상태: 설계 제안, 구현·모델 성능 검증 전.
...
**자주 실행하는 저비용 화면 감시 → 필요한 영역만 OCR → 발화 단위 ASR → 출처가 있는 근거 타임라인 → 맥락 기반 비파괴 전처리 → 구간화 → 구간별 요약 → 전체 학습 노트 합성**으로 구성한다.
...
| `summary.js` | 타임라인 전처리·선별, 구간 생성, 선택한 API 호출, 구간 요약·전체 합성, 결과 스키마 검사 | 요청 중 텍스트와 구조화된 요약 |
...
Whisper는 q8 계열을 품질 기준으로 삼되 모듈별 정밀도와 실시간 처리량을 함께 비교한다. 20초 한국어 기술강의 합성음의 동일 기기 실측에서 q8 인코더/q8 디코더는 RTF 1.79~2.04, q8 인코더/q4 디코더는 RTF 0.46~0.54였고 후자를 기본 WebGPU 조합으로 선정했다. 이 결과는 한 합성 음원의 Worker 단독 반복 측정으로, 실제 강의의 정확도·짧은 발화 분할·OCR 동시 부하·장시간 캡처 안정성을 보장하지 않는다. 실제 강의 백로그 재발에 따라 일반 무음 경계의 청크를 최소 10초까지 모으고 최대 20초에서 분할한다. 정지·탐색·배속 전환의 잔여 음성은 길이에 관계없이 내보낸다. 형식·정밀도 이름은 실제 모델 파일과 런타임 버전으로 고정한다. WebGPU가 없거나 device loss가 발생하면 미리 선택한 로컬 저사양 모드로만 전환하거나 중단한다. GPU 사용이 가능하다는 이유만으로 large-v3-turbo를 자동 다운로드하지 않는다.
...
```json
{
  ...
  "status": "confirmed",
  "selection": "included",
  "selectionReason": "핵심 개념의 정의와 적용 조건",
  "supersedes": ["ev-99"],
  "relatedEvidenceIds": ["asr-61"]
}
```

이것은 구현할 데이터 형태의 예시다. bbox는 ROI 내 정규화 좌표이며 ASR에는 없을 수 있다. confidence를 출력하지 않는 모델에는 숫자를 만들어 넣지 않고 `null`로 둔다. 모델별 confidence 척도를 직접 비교하지 않는다. OCR `t0/t1`은 화면에 존재한 구간, ASR은 발화 구간이므로 같은 값이라고 가정하지 않는다. `selection`과 `selectionReason`은 원본 근거를 바꾸는 필드가 아니라 이번 요약 입력에 사용할지를 기록하는 전처리 메타데이터다.
...
### 8.1 타임라인 전처리와 맥락 기반 선별

전처리는 타임라인 원문을 삭제·수정하는 과정이 아니라, **전후 맥락에 따라 요약 입력 후보를 선별하는 비파괴적 단계**다. 모든 처리는 offscreen 세션 메모리에서 수행하고, 별도 파일·`chrome.storage`·캐시에 평문 중간 결과를 저장하지 않는다.

1. `epoch`, `t0`, `t1`을 기준으로 안정 정렬하고 OCR·ASR의 관련 근거를 연결한다.
2. NFC·공백·명백한 인식 경계만 정돈한다. 원문의 숫자·단위·기호·대소문자·부정·수식은 바꾸지 않는다.
3. 앞뒤 근거와 시간·출처·revision을 함께 보고 각 항목을 `included / filtered / uncertain`으로 분류한다.
4. `filtered`는 반복 인사, 학습 내용과 무관한 진행 안내, 이미 대표 근거가 있는 완전 중복, 문맥상 의미가 없는 OCR·VAD 잡음에 한정한다. 원본 근거와 ID·시각·출처·제외 사유는 세션 종료까지 유지한다.
5. 단독으로 불필요해 보이는 문장이라도 개념 전환, 시험 힌트, 정정, 예외, 반례, 조건문과 연결되면 제외하지 않는다. 판단이 애매한 `uncertain` 항목은 요약 입력에 포함하고 낮은 확신도를 전달한다.

초기 구현은 별도 원격 LLM 전처리 호출을 만들지 않고 로컬 규칙과 인접 문맥으로 시작한다. 의미 판단을 확신할 수 없으면 삭제하지 않는 것이 기본값이다. 향후 모델 기반 선별을 추가하더라도 외부 텍스트 처리 동의를 다시 적용하고, 선택 결과와 이유를 검증 가능한 형태로 반환해야 한다.
...
### 9.2 전처리 후 전체 구간을 보존하는 요약

누적 원문 전체를 매번 보내거나 앞부분만 잘라 보내지 않는다. 처리 흐름은 **근거 타임라인 → 로컬 전처리 → 기존 방식의 구간화 → 구간별 구조화 요약 → 전체 학습 노트 합성 → 로컬 렌더링** 순서로 고정한다.

1. 8.1의 선별을 먼저 적용하되 `included`와 `uncertain`의 모든 근거 ID가 후속 처리 대상에 배정됐는지 확인한다.
2. 전처리된 근거를 슬라이드 변경·주제 구간·완결된 발화로 묶는다. 긴 구간은 대략 6k~12k 입력 토큰으로 나누되 실제 tokenizer와 모델 context 한도를 따른다.
3. 각 구간에서 `keyConclusions`, `concepts`, `claims`, `definitions`, `relationships`, `formulas`, `examples`, `corrections`, `openQuestions`, `importance`, `evidenceIds`를 구조화한다. 모호한 인식은 uncertainty에 남긴다.
4. 모든 구간별 구조화 결과로 전체 학습 노트를 만든다. 합성 입력이 한 번의 context 한도를 넘으면 구간 결과를 제한된 묶음으로 합성한 뒤 최종 통합하되, 각 단계에서 핵심 결론·수식·정정·예외·불확실성·근거 ID를 보존한다. 원문 산문을 계속 다시 요약하는 경로는 사용하지 않는다.
5. 전체 합성은 긴 강의에서도 생략하지 않는다. 호출 실패나 검증 실패 시 구간별 노트와 처리 상태를 보존하고 `partial`로 표시하며, 전체 노트가 완성됐다고 보고하지 않는다.

### 9.3 학습 보조 자료 프롬프트와 출력 계약

요약 요청에는 다음 지침을 시스템 메시지로 고정하고, OCR·ASR 텍스트와 분리한다.

> 입력된 타임라인 근거를 학습 보조 자료로 재구성한다. 전후 맥락을 고려하여 반복 인사, 무관한 진행 안내, 명백한 중복과 인식 잡음은 제외하되 제외 이유를 기록한다. 정정, 예외, 반례, 부정, 조건, 숫자, 단위, 기호와 수식은 우선 보존한다. 핵심 결론과 시험에 중요한 내용을 노트 맨 앞에 배치한다. 수식은 원식, 변수와 단위, 적용 조건, 직관적 의미를 설명하고, 근거가 충분할 때 표·도표·그래프로 구조화한다. 근거에 없는 수치나 관계를 만들어내지 않는다. 각 내용에는 근거 ID와 `핵심/중요/참고` 중요도 태그를 부여하고, 강의 원문을 장문으로 재현하지 않는다.

목표 출력 스키마에는 최소한 `keyConclusions`, `sections`, `formulas`, `visuals`, `reviewQuestions`, `evidenceIds`를 둔다. `formulas`는 LaTeX 원식·변수·단위·적용 조건·직관적 설명을 분리하고, `visuals`는 표·관계도·수치 그래프에 필요한 데이터와 출처를 구조화한다. 근거가 부족한 그래프는 만들지 않고 텍스트로 한계를 밝힌다.

색상은 모델이 임의 HTML·CSS를 생성하는 방식으로 제어하지 않는다. 모델은 `critical / important / reference` 같은 의미 태그만 반환하고, 사이드패널이 접근성을 확인한 고정 색상 토큰과 텍스트 라벨로 표현한다. 색만으로 중요도를 구분하지 않는다. 수식은 기존 KaTeX, 관계·흐름은 표와 Mermaid 기반 로컬 렌더링을 우선 재사용한다.
...
| 4. 근거·요약 | 출처·revision, 3상태 전처리, 전체 coverage 구간 요약, 전체 합성, 학습 노트 출력 계약, OpenRouter 연결 | 잘못된 선별·누락 구간·전체 합성 실패·근거 없는 요약을 평가 |
...
| 전처리 | 잘못 `filtered`된 유일 정보, 보호 항목 보존율, 제외 사유 coverage | 핵심 정정·예외·부정·숫자·단위·수식의 잘못된 제외 0건 목표; 모든 상태와 사유 추적 |
| 요약 | 사실 충실성·중요 내용 recall·근거 연결·후반부 포함·전체 합성 성공·수식/시각 자료 정확성 | 모든 처리 가능한 구간 포함, 핵심 결론 선두 배치, 근거 없는 핵심 주장·그래프 수치 0건 목표 |
...
이 작업에서는 `node --test lib/*.test.js`를 실행해 35개 테스트가 통과했다. 제안한 모델·캡처 구조·임계값의 실측 통과를 의미하지 않는다. 이번 변경은 검토용 문서와 설계도에 한정된다.
...
Archify `showcase` 명세 검증은 9/9 통과, 오류·경고 0건이다. Chrome 자동 배치 검사는 1440×900, 1600×1000, 1920×1080, 2048×1320에서 통과했다. 최종 1440×900·2048×1320의 밝은/어두운 테마 PNG 네 장을 직접 열어 글자·연결선·가림·배치를 확인했다. 첫 브라우저 검사에서 1440×900 화면이 긴 제목 줄바꿈으로 2px 넘친 문제는 제목을 압축한 뒤 다시 생성·검사했다. 검색·모든 탐색 버튼·내보내기 동작을 수동으로 전수 시험한 것은 아니다.
...
```text
spec sha256: e26e13262826f8fd3095e1cba73013b32436af7e0bda3e9f22aac1ae69f75d3d
html sha256: a41cd9df4c1ba9572c91f96f8fc75885e51cc2252b8c90a3b98524e7b73d0413
```
