...
function buildNotesPrompt(script, truncated, isLocal = false) {
  ...
  if (format === "summary") {
    if (isLocal) {
      instruction =
        ...
        "다음 작성 지침을 반드시 준수해라:\n" +
        "1. 구조화: 대제목(#), 중제목(##), 소제목(###), 글머리 기호를 활용해 계층적으로 읽기 쉽게 정리할 것.\n" +
        "2. 비교 및 정리: 대안 비교, 주요 항목, 수치 데이터는 마크다운 표(| 구분 | 내용 |)를 적극 활용할 것.\n" +
        "3. 수식(Math): 수식이나 계산식이 나오는 경우 일반 텍스트 또는 표준 LaTeX($수식$)로 명확히 적을 것.\n" +
        "4. 시각화/다이어그램 금지: ```mermaid 코드 블록이나 그래프 문법을 절대 작성하지 말고, 단계별 흐름이나 인과관계는 숫자 목록(1, 2, 3) 또는 글머리 기호(-, *)로 단계별로 명확히 서술할 것.\n" +
        "5. 이미지 금지: ![]() 형태의 이미지 마크다운 태그나 외부 이미지 링크를 절대 생성하지 말 것.\n" +
        "6. 핵심 원칙: 시험에 자주 출제되거나 실무에서 주의해야 할 핵심 원칙과 함정을 별도 섹션으로 강조할 것.";
    } else {
      instruction =
        "이 스크립트를 바탕으로 강의 내용을 잘 구조화된 한국어 핵심 학습 노트로 작성해라.\n" +
        "다음 작성 지침을 반드시 준수해라:\n" +
    ...
    instruction = `다음 조건을 반드시 지켜서 작성해라:\n조건: ${custom || '내용 요약'}` +
      (isLocal ? "\n(주의: ```mermaid 코드 블록이나 ![]() 이미지 태그는 생성하지 마라)" : "");
  }

  const drawingInstruction = isLocal
    ? `"[그림]" 으로 시작하는 부분은 화면에 있던 그래프·표·다이어그램을 옮긴 설명이다. ` +
      `그 내용을 읽기 쉬운 마크다운 표나 글머리 기호 목록으로 되살려라. 이미지 태그(![])나 mermaid 코드는 절대 쓰지 말고, 원문에 없는 수치를 지어내지 마라.`
    : `"[그림]" 으로 시작하는 부분은 화면에 있던 그래프·표·다이어그램을 옮긴 설명이다. ` +
...
async function notesLocal(full, onProgress) {
  ...
  try {
    const rawResult = await summarizeLocal(
      s,
      full,
      (part, isPartial) =>
        isPartial
          ...
          : buildNotesPrompt(part, false, true),
      onProgress
    );
    // 온디바이스 Nano의 환각 방어: 혹시 생성된 가짜 이미지 태그(![...](...))가 있다면 텍스트 레이블로 치환
    return (rawResult || "").replace(/!\[([^\]]*)\]\([^)]+\)/g, "🖼️ [$1]");
  } finally {
